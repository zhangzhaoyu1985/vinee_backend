<?php
namespace Services\WineMateServices;
...
# Default running mode to "prod", in dev mode pass in "dev" as parameter: php WineMateServicesHandler.php dev
$mode = "prod";
if ($argc > 1) {
  $mode = $argv[1];
}
...
class WineMateServicesHandler implements \Services\WineMateServices\WineMateServicesIf {

  public function login(\Services\WineMateServices\User $user) {
    //var_dump($user);
    $response = new \Services\WineMateServices\LoginResult;
    ...
    if ($result && $result->num_rows == 1) {
	  ...
	  if ($row['status'] == true) {
	    ...
	    print(intval(\Services\WineMateServices\LoginStatus::LOGIN_SUCCESS));
	    $response->status = \Services\WineMateServices\LoginStatus::LOGIN_SUCCESS;
	  } else {
	    print("log in user unactivated , userName = ".$user->userName."\n");
	    print(intval(\Services\WineMateServices\LoginStatus::LOGIN_UNACTIVATED));
	    $response->status = \Services\WineMateServices\LoginStatus::LOGIN_UNACTIVATED;
      }
    ...
    print(intval(\Services\WineMateServices\LoginStatus::LOGIN_FAILED));
    $response->status = \Services\WineMateServices\LoginStatus::LOGIN_FAILED;
    return $response;
  }
		
  public function loginWechat(\Services\WineMateServices\WechatLoginInfo $wechatLoginInfo) {
        ...
        if ($result && $result->num_rows == 1) {
	        ...
	        $sql = sprintf("UPDATE user_account_info SET first_name = '%s', head_image_url = '%s', sex = '%s', third_party = %d".
						   ...
						   $sex,
                           \Services\WineMateServices\ThirdParty::WECHAT,
	                       $row['user_id']);
	        print($sql."\n");
	        $conn->query($sql);
        } else {
  ...
  public function registration(\Services\WineMateServices\User $user) {
    if(empty($user->userName) || empty($user->email) || empty($user->password)) {
      return \Services\WineMateServices\RegistrationStatus::REGISTRATION_INVALID_INPUT;
    }
    ...
    if ($res_find_name && $res_find_name->num_rows > 0) {
      ...
      return \Services\WineMateServices\RegistrationStatus::REGISTRATION_DUPLICATE_USERNAME;
    }
    ...
    if ($res_find_email && $res_find_email->num_rows > 0) {
      ...
      return \Services\WineMateServices\RegistrationStatus::REGISTRATION_DUPLICATE_EMAIL;
    }
    ...
    return \Services\WineMateServices\RegistrationStatus::REGISTRATION_SUCCESS;
  }
  ...
  public function findPassword(\Services\WineMateServices\User $user) {
    print "find password request received";
    if (empty($user->userName) && empty($user->email)) {
      return \Services\WineMateServices\FindPasswordStatus::PW_FAILED;
    }
    ...
    if ($result && $result->num_rows == 1) {
      ...
      return \Services\WineMateServices\FindPasswordStatus::PW_SUCCESS;
    }

    return \Services\WineMateServices\FindPasswordStatus::PW_FAILED;
  }

  public function authentication(\Services\WineMateServices\TagInfo $tag, $userId) {
    $wineInfo = new \Services\WineMateServices\WineInfo;
    $conn = db_connect(true);
    var_dump($tag);
    ...
    if ($sql_result && $sql_result->num_rows > 0) {
      ...
      $sql_wine_info = "";
      if ($tag->countryId == \Services\WineMateServices\CountryId::ENGLISH) {
	...
	$wineInfo->wineryLogoPicUrl = $row['winery_logo_pic_url'];
      ...
      $ratingRequest = new \Services\WineMateServices\WineReviewAndRatingReadRequest;
      ...
      $wineInfo->rewardPoint = getRewardPointsByActionType(\Services\WineMateServices\UserActions::OpenedBottle, $wineInfo->wineId);       
  ...
  public function openBottle(\Services\WineMateServices\BottleOpenInfo $openInfo) {
  ...
  public function getBasicInfo(\Services\WineMateServices\WineBasicInfoRequest $request) {
    $response = new \Services\WineMateServices\WineBasicInfoResponse;
    //var_dump($request);
    ...
    if ($sql_result && $sql_result->num_rows == 1) {
      ...
      if (!empty($row['food_paring_pic_text']) && !empty($row['food_paring_pic_url'])) {
        ...
        if(sizeof($urlVec) == $size && $size > 0) {
          for ($x = 0; $x < $size; $x++) {
            $foodParingInfo = new \Services\WineMateServices\FoodParingPics;
  ...
  public function getWineReviewAndRating(\Services\WineMateServices\WineReviewAndRatingReadRequest $request) {
    //var_dump($request);
    $response = new \Services\WineMateServices\WineReviewAndRatingReadResponse;
    $response->data = array();
    $conn = db_connect();
...
/*
    if ($sql_result && $sql_result->num_rows > 0) {
      ...
      for ($i = 0; $i < $sql_result->num_rows; $i++){
        $row = $sql_result->fetch_assoc();
        $data = new \Services\WineMateServices\WineReviewAndRatingData;
        ...
        if ($result_reviewer && $result_reviewer->num_rows > 0) {
          ...
          $data->sex = ($row_reviewer['sex'] == "m" ? (\Services\WineMateServices\ReviewerSex::MALE) : (\Services\WineMateServices\ReviewerSex::FEMALE));
        }
  ...
  public function writeWineReviewAndRating(\Services\WineMateServices\WineReviewAndRatingWriteRequest $request) {
    //var_dump($request);
    $response = new \Services\WineMateServices\WineReviewAndRatingWriteResponse;
  ...
  public function getMyRateRecord(\Services\WineMateServices\MyRateRecordRequest $request) {
    //var_dump($request);

    $response = new \Services\WineMateServices\MyRateRecordResponse;
    $conn = db_connect();
  ...
  public function getMyOpenedBottles(\Services\WineMateServices\MyBottlesRequest $request) {
    $response = new \Services\WineMateServices\OpenedBottlesResponse;
    ...
    if($sql_result->num_rows > 0) {
      $row = $sql_result->fetch_assoc();
      $response->sex = ($row['sex'] == 'm' ? \Services\WineMateServices\ReviewerSex::MALE : \Services\WineMateServices\ReviewerSex::FEMALE);
	  $response->photoUrl = $row['head_image_url'];
    } else {
      $response->sex = \Services\WineMateServices\ReviewerSex::MALE;
    }
    ...
    for($i = 0; $i < $sql_result->num_rows; $i++) {
      $row = $sql_result->fetch_assoc();
      $wineId = $row['wine_id'];
      $data = new \Services\WineMateServices\BottleInfo;
      $data->wineId = $wineId;
  ...
  public function getMyScannedBottles(\Services\WineMateServices\MyBottlesRequest $request) {
    var_dump($request);
  ...
  public function getMyNewsFeed(\Services\WineMateServices\NewsFeedRequest $request) {
    //var_dump($request);
    $response = new \Services\WineMateServices\NewsFeedResponse;
    ...
    for ($i = 0; $i < $sql_result->num_rows; $i++) {
      $row = $sql_result->fetch_assoc();
      $data = new \Services\WineMateServices\NewsFeedData;
      $data->feedType = \Services\WineMateServices\FeedType::USERFEED;
      ...
      if (is_array($user_info)) {
        ...
        if ($request->countryId == \Services\WineMateServices\CountryId::ENGLISH) {
      ...
      $bottle_info = new \Services\WineMateServices\BottleInfo;
  ...
  public function getWineryInfo(\Services\WineMateServices\WineryInfoRequest $request) {
    //var_dump($request);
    $response = new \Services\WineMateServices\WineryInfoResponse;
    ...
    for($i = 0; $i < $wine_info_result->num_rows; $i++) {
      $row = $wine_info_result->fetch_assoc();
      $item = new \Services\WineMateServices\WineryInfoResponseSingleItem;
    ...
    if ($request->countryId == 1) {
      ...
      $sql = "SELECT * FROM content_info_Chinese WHERE content_id = ".$content_id;
    }
    $sql_result = $conn->query($sql);
    ...
    for($i = 0; $i < $sql_result->num_rows; $i++) {
      $row = $sql_result->fetch_assoc();
      $single_content = new \Services\WineMateServices\WineryInfoSingleContent;
  ...
  public function getFriendList(\Services\WineMateServices\FriendListRequest $friendListRequest) {

    $userId = $friendListRequest->userId;
    $response = new \Services\WineMateServices\FriendListResponse;
    $conn = db_connect();
    ...
    foreach($friendInfoArray as $friendInfo) {
      $data = new \Services\WineMateServices\FriendInfo;
  ...
  public function getMyFollowingList(\Services\WineMateServices\FriendListRequest $request) {
    $userId = $request->userId;
    $response = new \Services\WineMateServices\MyFollowingListResponse;
  ...
  public function getMyFollowersList(\Services\WineMateServices\FriendListRequest $request) {
    $userId = $request->userId;
    $response = new \Services\WineMateServices\MyFollowersListResponse;
  ...
  public function getMyProfile($observerId, $userId) {
    $response = new \Services\WineMateServices\MyProfile;
    $userStruct = new \Services\WineMateServices\User;
  ...
  public function updateMyProfile(\Services\WineMateServices\User $userStruct) {
    return true;
  }
  ...
  public function addRewardPoints(\Services\WineMateServices\AddRewardPointsRequest $addRewardPointsRequest) {
    $conn = db_connect(true);
     var_dump($addRewardPointsRequest);
    $response = \Services\WineMateServices\AddRewardPointsResponse::AlreadyEarned;
    $userId = $addRewardPointsRequest->userId;
    $actionType = $addRewardPointsRequest->useAction;
    $wineId = $addRewardPointsRequest->wineId;
    $objectId = ($actionType == \Services\WineMateServices\UserActions::ShareWineInfoToWechat ? $wineId : 0);
    ...
    $response = \Services\WineMateServices\AddRewardPointsResponse::Success;
    $conn->query($sql);
    return $response;
  }
  ...
  public function getMyWishlist(\Services\WineMateServices\MyBottlesRequest $request) {
    $response = new \Services\WineMateServices\MyWishListResponse;
  ...
  public function addToWishlist(\Services\WineMateServices\AddToWishlistRequest $request) {
  ...
  public function isInWishlist($userId, $wineId) {
    $response = new \Services\WineMateServices\IsInWishlistResponse;
	$conn = db_connect();
  ...
  public function getUserPhoto($userId){
    $response = new \Services\WineMateServices\UserPhotoResponse;
  ...
  public function getRewardItemList(\Services\WineMateServices\RewardItemRequest $request) {
    $response = new \Services\WineMateServices\RewardItemResponse;
    ...
    $wine_info_table_name = ($request->countryId == \Services\WineMateServices\CountryId::ENGLISH ? 'wine_basic_info_english' : 'wine_basic_info_chinese');
    ...
    for($i = 0; $i < $sql_result->num_rows; $i++) {
      ...
      $rewardItem = new \Services\WineMateServices\RewardSingleItem;
  ...
  public function rewardRedeem(\Services\WineMateServices\RewardRedeemRequest $request) {
    $response = new \Services\WineMateServices\RewardRedeemResponse;
    ...
    if ($sql_result && $sql_result->num_rows > 0) {
      $row = $sql_result->fetch_assoc();
      if ($row['status'] != true) {
        $response->resp_code = \Services\WineMateServices\RewardRedeemResponseCode::ACCOUNT_UNACTIVATED;
        return $response;
      }
    } else {
      $response->resp_code = \Services\WineMateServices\RewardRedeemResponseCode::FAILED;
      return $response;
    }
    ...
    $response->resp_code = \Services\WineMateServices\RewardRedeemResponseCode::SUCCESS;
  ...
  public function getAllBottles(\Services\WineMateServices\AllBottlesRequest $request) {
    var_dump($request);
    $response = new \Services\WineMateServices\AllBottlesResponse;
...
if ($mode == "dev") {
  ...
  $handler = new WineMateServicesHandler();
  $processor = new \Services\WineMateServices\WineMateServicesProcessor($handler);

  $serverTransport = new TServerSocket('0.0.0.0',7891);
  //$serverTransport = new TServerSocket('localhost',7891); // For local debug
