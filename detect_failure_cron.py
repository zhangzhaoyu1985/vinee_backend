...
#     * * * * * /usr/bin/python /home/ubuntu/TagTalkServices/detect_failure_cron.py
...
f = open('/tmp/server_health_log.txt', 'a')

# list all currently running php processes.
ps_out = commands.getstatusoutput("ps -o cmd= -C php")[1]

dead_time = strftime("%Y-%m-%d %H:%M:%S", gmtime())

# if no php process with running command contains 'PhpServer.php', server is dead.
if (ps_out.find("WorkerMan") == -1):
  # Restart server and write log.
  f.write('server dead:      ' + dead_time + ' UTC\n')
  p = subprocess.Popen(['/usr/bin/php', os.path.expanduser('/home/ubuntu/ProdTagTalkServices/start.php'), 'stop'])
  p = subprocess.Popen(['/usr/bin/php', os.path.expanduser('/home/ubuntu/ProdTagTalkServices/start.php'), 'start', '-d'])
  f.write('server restarted: ' + strftime("%Y-%m-%d %H:%M:%S", gmtime()) + ' UTC\n')
  ...
  msg = MIMEText("Anyone messing around our server (physical address: " + socket.gethostbyname(socket.gethostname()) + ")? >.^\n\nThe cron job that runs every munite detected our workerman server process is dead on: " + dead_time + "\n\nIt just restarted the server, however please still check server health ASAP!")
  msg['Subject'] = "TagTalk Workerman Server DEAD and RESTARTED!!!";
  msg['From'] = sender 
